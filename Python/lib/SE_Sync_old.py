...
def SE_Sync(measurements, Manopt_opts=None, SE_Sync_opts=None, Y0=None):
    ...
    manopt_data['M'] = pymanopt.manifolds.Stiefel(problem_data['n'], problem_data['d'])
    ...
    for r in range(SE_Sync_opts['r0'], SE_Sync_opts['rmax'] + 1):
        ...
        optimization_times[iter - 1] = manopt_info[-1]['time']
        
        # Store gradient norm and state traces
        gradnorms.append(manopt_info['gradnorm'])
        Yvals.append(manopt_info['Yvals'])
        ...
        if lambda_min > -SE_Sync_opts['min_eig_num_tol']:
            ...
            manopt_data['M'] = manopt.manifolds.StiefelStacked(problem_data['n'], problem_data['d'], r+1)
            ...
            stepsize, Y0T = manopt.optimize.linesearch_decrease(manopt_data, Yplus.T, alpha * Ydot.T, SDPLRval)
            line_search_time = time.time() - line_search_time_start 
            Y0 = Y0T.T
            print('Line search completed (elapsed computation time {} seconds)\n'.format(line_search_time))
    ...
    print('Norm of Riemannian gradient grad F(Y): {}'.format(manopt_info[-1]['gradnorm']))
